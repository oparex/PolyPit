"""
place_order.py — Place a market order on the current BTC 5-minute Up/Down market.
...
def main():
    ...
    while True:
        amount_raw = input(
            "Amount (USD for BUY, shares for SELL): "
        ).strip()
        try:
    ...
    ref_str = f"{ref_price:.4f}" if ref_price is not None else "unknown"
    print(f"\n  Outcome  : {outcome_name}")
    print(f"  Token ID : {token_id}")
    print(f"  Side     : {side}")
    print(f"  Amount   : {amount}")
    print(f"  Ref price: {ref_str} (best {'ask' if side == 'BUY' else 'bid'})")
    ...
    try:
        market_price = client.calculate_market_price(token_id, side, amount, OrderType.FOK)
        print(f"  Calculated price: {market_price:.4f}")
    except Exception as exc:
        print(f"[error] Could not calculate market price: {exc}")
        return
